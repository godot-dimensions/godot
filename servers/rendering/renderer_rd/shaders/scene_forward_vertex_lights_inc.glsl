...
void light_process_omni_vertex(uint idx, vec3 vertex, float vertex_w, hvec3 eye_vec, hvec3 normal, half roughness,
		inout hvec3 diffuse_light, inout hvec3 specular_light) {
	vec3 light_rel_vec = omni_lights.data[idx].position - vertex;
	float light_length = length(light_rel_vec);
	hvec3 light_rel_vec_norm = hvec3(light_length > 0.0 ? light_rel_vec / light_length : vec3(0.0));
	half omni_attenuation = get_omni_attenuation(light_length, omni_lights.data[idx].inv_radius, omni_lights.data[idx].attenuation, omni_lights.data[idx].slice_offset - vertex_w);
...
void light_process_spot_vertex(uint idx, vec3 vertex, float vertex_w, hvec3 eye_vec, hvec3 normal, half roughness,
