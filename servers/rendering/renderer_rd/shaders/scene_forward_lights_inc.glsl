...
void light_compute(vec3 N, vec3 L, vec3 V, float A, vec3 light_color, bool is_directional, float attenuation, float slice_component, vec3 f0, uint orms, float specular_amount, vec3 albedo, inout float alpha,
...
void light_process_omni(uint idx, vec3 vertex, float vertex_w, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float shadow, vec3 albedo, inout float alpha,
#ifdef LIGHT_BACKLIGHT_USED
		vec3 backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
	...
	light_compute(normal, light_rel_vec_norm, eye_vec, size_A, color, false, light_attenuation, slice_component, f0, orms, omni_lights.data[idx].specular_amount, albedo, alpha,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
...
void light_process_spot(uint idx, vec3 vertex, float vertex_w, vec3 eye_vec, vec3 normal, vec3 vertex_ddx, vec3 vertex_ddy, vec3 f0, uint orms, float shadow, vec3 albedo, inout float alpha,
#ifdef LIGHT_BACKLIGHT_USED
		vec3 backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
	...
	float scos = max(get_spot_cosine(light_rel_vec_norm, slice_component, spot_dir, spot_lights.data[idx].slice_direction), cone_angle);
	...
	light_compute(normal, light_rel_vec_norm, eye_vec, size_A, color, false, light_attenuation, slice_component, f0, orms, spot_lights.data[idx].specular_amount, albedo, alpha,
#ifdef LIGHT_BACKLIGHT_USED
			backlight,
#endif
#ifdef LIGHT_TRANSMITTANCE_USED
